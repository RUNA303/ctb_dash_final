...
st.set_page_config(
    page_title="CitiBike Strategy Dashboard (dashdashdash.py)",
    page_icon="🚲",
    layout="wide"
)

# --- Title and Description ---
st.title("Citi Bikes Strategy Dashboard") # Modified title based on templateStud.txt

### DEFINE SIDE BAR
st.sidebar.title('Aspect Selector') # Modified title based on templateStud.txt
page = st.sidebar.selectbox('Select an aspect of the analysis',
                            ['Intro page',
                             'Weather component and bike usage',
                             'Most popular stations',
                             'Bike-type usage aligned to the Temperature', # Corrected: Added comma
                             'Interactive map with aggregated bike trips',
                             'Recommendations']) # Removed 'Classic versus electric bikes' as it's not a required page in the task

################################################# IMPORT DATA #####################################################################

# --- Dropbox URL для processed_data.csv ---
# Используйте предоставленную ссылку с параметром ?dl=0 или ?dl=1 для прямого скачивания
csv_file_url = 'https://dl.dropbox.com/scl/fi/7vg7i8kxgkjk29srbl5qo/processed_data.csv?rlkey=qe5zx4kmogd78q4niqvttqkve&st=x7y4a9th&dl=0'


@st.cache_data
def load_data_from_url(url): # Переименовал функцию для ясности
    try:
        # Скачиваем файл по URL
        response = requests.get(url, stream=True) # stream=True для потенциально больших файлов
        response.raise_for_status() # Выбросить исключение для плохих статусов (4xx или 5xx)

        # Читаем данные прямо из ответа (потока)
        # pandas может читать прямо из ответа requests с параметром stream=True
        df = pd.read_csv(response.raw, index_col=0, compression='gzip', sep=',')

        # При необходимости приводим столбцы к нужным типам после загрузки
        if 'date' in df.columns:
             df['date'] = pd.to_datetime(df['date']) # Убрал формат, pandas часто определяет его сам
        for col in ['start_lat', 'start_lng', 'end_lat', 'end_lng']:
             if col in df.columns:
                 df[col] = pd.to_numeric(df[col], errors='coerce')
        # df.dropna(subset=['start_lat', 'start_lng'], inplace=True) # Drop rows where essential geo-data is missing, if needed

        return df
    except requests.exceptions.RequestException as e:
        st.error(f"Ошибка при скачивании данных с Dropbox: {e}")
        return None
    except Exception as e:
        st.error(f"Произошла ошибка при загрузке или обработке данных: {e}")
        return None

# Вызываем функцию для загрузки данных с Dropbox
df = load_data_from_url(csv_file_url)

# Prepare top20 data if df loaded successfully
top20 = pd.DataFrame() # Initialize as empty DataFrame
if df is not None:
    # Calculate trip counts by start station - CORRECTED in previous turn
    if 'start_station_name' in df.columns:
        ...
        st.warning("Column 'start_station_name' not found in data. Cannot prepare top stations data.")
...
if page == 'Intro page':
    ...
    try:
        # Assuming the image file is named
        myImage = Image.open("citi_bike_nyc_AP18117005853119.jpeg") # Эта строка будет заменена
        ...
        st.error("Image not found: citi_bike_nyc_AP18117005853119.jpeg. Make sure this image is in your GitHub repo.")
...
### LINE CHART PAGE: WEATHER COMPONENT AND BIKE USAGE
    ...
    st.header("Daily Bike Trips and Temperatures") # Modified header based on templateStud.txt

    if df is not None and 'avgTemp' in df.columns and 'date' in df.columns:
        # --- Line Chart Data Wrangling ---
        # Calculate daily trip counts - CORRECTED
        daily_rides = df.groupby('date').size().reset_index(name='trip_count')
        # Assuming 'avgTemp' is the correct column name for average temperature
        ...
        if not df_line.empty:
            ...
            fig_line.add_trace(go.Scatter(x=df_line['date'], y=df_line['trip_count'], name='Daily bike rides', marker={'color': 'blue'}), secondary_y=False,) # Modified name and color based on templateStud.txt, using 'trip_count'
            fig_line.add_trace(go.Scatter(x=df_line['date'], y=df_line['avgTemp'], name='Daily temperature', marker={'color': 'red'}), secondary_y=True,) # Modified name and color based on templateStud.txt
            fig_line.update_layout(
                title='Daily bike trips and temperatures in 2022', # Modified title based on templateStud.txt
                height=400 # Based on templateStud.txt
            )
        ...
        st.warning("Required columns ('avgTemp' or 'date') not found in the dataset for the weather chart.")
...
### BAR CHART PAGE: MOST POPULAR STATIONS
    ...
    st.header("Most Popular Start Stations") # Modified header

    if df is not None and not top20.empty:
        # --- Display Total Rides Metric ---
        # Calculate total rides from the top 20 data - CORRECTED
        total_rides = float(top20['trip_count'].sum())
        st.metric(label='Total Bike Rides', value=numerize.numerize(total_rides)) # Based on templateStud.txt

        # --- Create Bar Chart ---
        fig_bar = go.Figure( # Changed from px.bar to go.Figure for more control over orientation and color
            go.Bar(
                x=top20['trip_count'], # Swapped x and y for horizontal bar chart, using 'trip_count' - CORRECTED
                y=top20['start_station_name'],
                orientation='h', # Set orientation to horizontal
                marker={'color': top20['trip_count'], 'colorscale': 'Blues'} # Added color scale based on templateStud.txt, using 'trip_count' - CORRECTED
            )
        )
        fig_bar.update_layout(
            title='Top 20 most popular bike stations', # Modified title based on templateStud.txt
            xaxis_title='Sum of trips', # Modified axis title based on templateStud.txt
            yaxis_title='Start stations', # Modified axis title based on templateStud.txt
            width=900, # Based on templateStud.txt
            height=600 # Based on templateStud.txt
        )
        st.plotly_chart(fig_bar, use_container_width=True)
    elif df is None:
         st.warning("Data not loaded. Cannot display bar chart.")
    else:
        st.warning("Top stations data not available. Check data loading and preparation.")
...
### MAP PAGE: INTERACTIVE MAP WITH AGGREGATED BIKE TRIPS
    ...
    # --- Dropbox URL for nyc_bike_trips_map.html ---
    # ?dl=0 или ?dl=1
    map_html_url = 'https://dl.dropbox.com/scl/fi/9fkjdes6z9xf4kvphy4zf/nyc_bike_trips_map.html?rlkey=14s1wv7yebxbx6siamq4vqk6c&st=apalrwrq&dl=0'


    try:
        # Скачиваем HTML содержимое по URL
        response = requests.get(map_html_url)
        response.raise_for_status() 

        html_data = response.text # Получаем HTML как текст

        st.components.v1.html(html_data, height=600)

    except requests.exceptions.RequestException as e:
        st.error(f"Ошибка при скачивании файла карты с Dropbox: {e}")
    except Exception as e:
        st.error(f"An error occurred while fetching or displaying the Kepler map: {e}")
...
else: # This 'else' will catch the 'Recommendations' page selection
    st.header("Conclusions and Recommendations") # Modified header based on templateStud.txt
    try:
        bikes = Image.open("14820.jpg") # Эта строка будет заменена
        st.image(bikes, caption="Photo from Freepik")
    except FileNotFoundError:
        st.error("Image not found: 14820.jpg. Make sure this image is in your GitHub repo.")
